resource "azurerm_network_interface" "nic" {
  ...
  ip_configuration {
    name                          = "internal"
    subnet_id                     = var.subnet_id[each.key] # Ensure subnet_id is a map
    private_ip_address_allocation = "Dynamic"
  }
}

resource "azurerm_network_security_group" "s2_nsg" {
  for_each = var.v2_nsg_name # Ensure nsg_config is a map in variables.tf
  name                = each.value.name
  location            = var.location
  resource_group_name = var.resource_group_name

  dynamic "security_rule" {
    for_each = var.nsg_rules # Use proper nested structure
    iterator = rule
    content {
      name                       = rule.key
...
resource "azurerm_network_interface_security_group_association" "nic_nsg_ass" {
  ...
  network_security_group_id = azurerm_network_security_group.s2_nsg[each.key].id
